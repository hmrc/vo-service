...
/**
  ...
  */
class SecondaryButtonSpec extends BaseAppSpec:

  "SecondaryButton" should {
    "implement the required properties of the parent Button class" in {
      val button: Button = SecondaryButton()
      button.id                 shouldBe Some("save-button")
      button.name               shouldBe Some("save-button")
      button.value              shouldBe Some("save-button")
      button.inputType          shouldBe Some("submit")
      button.isStartButton      shouldBe false
      button.content            shouldBe Text("button.save.label")
      button.preventDoubleClick shouldBe Some(true)
      button.classes            shouldBe "govuk-button--secondary"
