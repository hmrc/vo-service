...
class StartButtonSpec extends BaseAppSpec:
  ...
  "StartButton" should {
    "implement the required properties of the parent Button class" in {
      val button: Button = StartButton("/some/url")
      button.id            shouldBe Some("start-button")
      button.isStartButton shouldBe true
      button.content       shouldBe Text("button.start.label")
